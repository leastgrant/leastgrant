...
- **Google Antigravity.** The hook engine installs only when the server-delivered experiment flag json-hooks-enabled is true. It is per-session, not persisted, not readable and not overridable from the client, so LeastGrant cannot tell an enforcing session from an unenforced one. force_ask is silently downgraded to an allow when the host sets auto_interaction_behavior=ALLOW_ALL. ResolveAutoInteraction runs before any prompt is registered and the hook gets no signal. A non-zero exit discards the hook’s stdout entirely and a failed hook fails OPEN — applyPreToolHooks logs the error and proceeds to the next hook, leaving the tool call to run. deny and ask do not have the same reach. deny is enforced in the tool-call converter, before the permission manager; ask and force_ask are read only by the permissions package, which a step reaches only if it declares permission targets. The runtime reads hooks from four workspace roots — .agents, _agents, .agent and _agent — and from plugins/<name>/hooks.json. Any of them can add a handler; the published docs mention only .agents. An unrecognised non-empty decision string is not a deny. The switch falls through to the engine’s own permission check, so a typo in a third-party hook degrades to the default rather than to safety. Two of the six accepted verdicts are undocumented. auto_approve is accepted by the runtime and returns a silent allow with no prompt, and appears in neither the published docs nor the hooks guide embedded in the binary; deny_unless_prior_grant appears on the web page but not in that embedded guide. The runtime supports six hook events — PreToolUse, PostToolUse, PreInvocation, PostInvocation, Stop and SessionStart. Stop carries its own decision field. LeastGrant registers only the two tool events, so the others are unmediated. Prompt-mode handlers exist (HookHandlerConfig.prompt, executePromptModeHook) despite the embedded guide saying only command handlers are supported. A prompt handler is a model deciding a permission question. Never run inside a real Antigravity session. The contract is read from the shipped 2.11.0 runtime; nothing has exercised it. Which tool-name namespace PreToolUse matchers see is not settled. The runtime holds two — the model-facing names (write_to_file, list_dir, find_by_name) and names derived by lowercasing the CORTEX_STEP_TYPE_ enum (propose_code, list_directory, find). They coincide for run_command, view_file and grep_search and diverge for exactly the file-writing tools, so a smoke test passes while write gating silently does not. LeastGrant maps the model-facing names, which the documented PreToolUse example supports; only a live write can settle it. LeastGrant does not model the PreInvocation, PostInvocation, Stop or SessionStart events, and does not use the overwrite field that lets a hook rewrite a tool call’s arguments.
