...
- **Cursor.** File reads are observed after the fact, not gated. The beforeReadFile payload carries the file content, so the read has already happened by the time LeastGrant sees it. A deny stops the content reaching the model. Writes, edits and deletes have no pre-execution event and are not intercepted at all. Measured: a file creation produced zero hook invocations. beforeReadFile takes allow or deny and has no ask, so an unfamiliar read is allowed rather than questioned. Only a read LeastGrant recognises as a credential is blocked. No permission mode reaches the hook, so LeastGrant cannot tell an attended session from an unattended one. Cursor also ingests Claude Code hooks from .claude/settings.json and ~/.claude/settings.json, so installing both adapters may double-fire. Cursor has its own sandbox and its own auto-run flow, and both act before the user sees anything. A command may be attempted sandboxed, blocked by that sandbox, and retried unsandboxed, with LeastGrant consulted separately each time. There is no equivalent of Antigravity’s force_ask: nothing LeastGrant can return compels a prompt that the host’s own settings cannot satisfy. MCP was not exercised in the live run — beforeMCPExecution is registered and was never invoked, so its behaviour is contract-derived. Verified on Windows only. The POSIX transport passes the payload on native stdin rather than through PowerShell and has not been exercised. The hook timeout was not tested; only the crash path was.
