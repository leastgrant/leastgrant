...
function fam(over: Partial<Familiarity> = {}): Familiarity {
  ...
  // `approvedDays` / `approvedSessions` default to the sighting counts here.
  //
  // Every case in this file that sets `days: 2, sessions: 2` alongside a
  // confirmed count means "the human approved this across two days", which is
  // what those fields meant when the cases were written. The engine now
  // separates the two, because sightings are generated by the agent itself and
  // cannot carry that meaning — see SignatureStat.approvedDays. Mirroring them
  // by default keeps each case asserting what its author intended, and a case
  // that wants them to differ says so explicitly.
  return {
    ...base,
