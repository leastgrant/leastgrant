...
export function render() {
  ...
  const rows = shipped.map((a) => {
    ...
    return `| **${a.name}** | ${STATUS[level] ?? level} | ${deriveVerification(a)} | ${cell(value(a, 'verdicts', 'allow'))} | ${cell(value(a, 'verdicts', 'ask'))} | ${cell(value(a, 'verdicts', 'deny'))} | ${crash === 'closed' ? 'refuses' : crash === 'open' ? 'runs anyway' : '?'} | ${a.versionTested ?? '—'}${(a.osTested ?? []).length ? ` on ${a.osTested.join(', ')}` : ', not run live'} |`;
  });

  // Only the grades actually in use, so the legend cannot grow a row for a
  // standard nothing here meets.
  const gradesInUse = [...new Set(shipped.map(deriveVerification))];
  ...
  const out = [
    ...
    '',
    ...gradesInUse.map((g) => `- **${g}** — ${GRADE_MEANING[g]}.`),
    '',
